...
pytest -q                        # ~2,267 collected, ~2,242 passed, ~25 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 124 modules) plus a large test suite (171 test files, ~2,267 tests). All modules append to and replay one hash chained event log:
