...
pytest -q                        # ~2,347 collected, ~2,320 passed, ~27 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 124 modules) plus a large test suite (161 test files, ~2,347 tests). All modules append to and replay one hash chained event log:
