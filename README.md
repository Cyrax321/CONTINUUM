...
pytest -q                        # ~2,528 collected, ~2,423 passed, ~28 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 124 modules) plus a large test suite (161 test files, ~2,528 tests). All modules append to and replay one hash chained event log:
...
| `cli/` | 47 argparse commands, exit codes as verdict: `runs, start, inspect, resume, verify, health, tree, benchmark, attest, dashboard, policy-review` |
