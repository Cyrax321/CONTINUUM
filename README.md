...
pytest -q                        # ~2,308 collected, ~2,283 passed, ~25 skipped on a minimal env (exact counts vary)
...
- **Scale**: roughly 2,308 tests collected (~2,283 passing; the rest skip without optional services) on Python 3.11, 3.12, and 3.13 (unit, `hypothesis` property-based, concurrency, adversarial). CONTINUUM-Bench runs five crash scenarios plus a dedicated argument-drift scenario, measuring 0 duplicate work and 0 duplicate side effects for CONTINUUM against full duplication for naive replay; a separate 15-scenario recovery-correctness suite (`continuum.benchmark.phase6`) encodes the crash points from the durable-execution survey as executable assertions, and an 8-fault risk-injection suite (`benchmarks/fault_injection/`) grades failure handling.
...
CONTINUUM is one library (`src/continuum`, 124 modules) plus a large test suite (161 test files, ~2,308 tests). All modules append to and replay one hash chained event log:
