...
pytest -q                        # ~2,279 collected, ~2,251 passed, ~27 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 128 modules) plus a large test suite (171 test files, ~2,279 tests). All modules append to and replay one hash chained event log:
