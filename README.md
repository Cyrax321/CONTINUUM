...
pytest -q                        # ~2,363 collected, ~2,337 passed, ~26 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 127 modules) plus a large test suite (172 test files, ~2,363 tests). All modules append to and replay one hash chained event log:
...
| `cli/` | 48 argparse commands, exit codes as verdict: `runs, start, inspect, resume, verify, health, tree, benchmark, attest, lineage-issue, lineage-verify, dashboard` |
