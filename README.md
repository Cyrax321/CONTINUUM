...
pytest -q                        # ~2,136 collected, ~2,030 passed, ~23 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 104 modules) plus a large test suite (98 test files, ~2,136 tests). All modules append to and replay one hash chained event log:
