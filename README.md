...
pytest -q                        # ~2,419 collected, ~2,391 passed, ~28 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 124 modules) plus a large test suite (161 test files, ~2,419 tests). All modules append to and replay one hash chained event log:
