...
pytest -q                        # ~2,465 collected, ~2,436 passed, ~28 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 124 modules) plus a large test suite (161 test files, ~2,465 tests). All modules append to and replay one hash chained event log:
