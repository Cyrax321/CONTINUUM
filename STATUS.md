...
| Checkpoints | `checkpoint/` | Policy-driven (manual, interval, event, semantic, context-pressure, hybrid). Restore replays events recorded after the checkpoint. Phase 4 added `RECOVERY` anchors (`checkpoint_on_recovery`), `last_recovery_anchor` lookup, `prune` (keeps newest + anchors), and `Storage.delete_checkpoint` (SQLite + Postgres); `StateCheckpoint.reason` is now stored. The CLI records an anchor on `continuum resume <run> --repair` after a non-RESUME verdict, and `continuum restore <run> --reason ... --to-recovery-anchor` rolls back to it (#1097); the engine itself stays read-only. |
