...
A maioria dos comandos aceita o modificador global `--json` **antes** do subcomando (por exemplo, `continuum --json resume RUN`); colocá-lo depois do subcomando é rejeitado. Comandos somente leitura não alteram o estado da execução; `resume` sem `--repair` ainda pode acrescentar eventos `NOTIFICATION_SENT` / `NOTIFICATION_FAILED` quando `.continuum/webhooks.json` está configurado. Códigos de saída são um contrato de segurança (apenas uma execução verificada segura sai com 0). Lista completa de comandos, tabela de códigos de saída e saída de diff de estado em [references/cli.md](references/cli.md).
