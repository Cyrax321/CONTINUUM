...
def test_cli_compact_accepts_env_flag(db: str) -> None:
    """The compact CLI accepts --env and records the supplied environment on the anchor (issue #1049)."""
    _seed_pinned_run(db)
    code, _out, err = run("--db", db, "compact", "run_1", "--force", "--env", "dataset=v4")
    assert code == ExitCode.OK, err
