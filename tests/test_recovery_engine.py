...
def test_risk_trigger_survives_compaction(store: SQLiteStorage) -> None:
    """A risk observed before the anchor must keep driving the verdict once
    compaction archives it out of the live tail. The risk scan folds the
    archived prefix, so ``triggering_risks`` cannot empty and a trigger the
    policy maps to rollback cannot silently downgrade to a less cautious mode
    (#1050)."""
    store.create_run(Run(run_id="r1", goal="do X"))
    store.append_event("r1", EventType.RUN_STARTED, {"goal": "do X"}, source=Origin.EXTERNAL_AGENT)
    store.append_event(
...
def test_liveness_breach_count_survives_compaction(store: SQLiteStorage) -> None:
    """A silence detected before the anchor must still count once compaction
    archives it. The breach count folds the archived prefix, so it cannot
    reset to zero and understate how often the run went quiet (#1050)."""
    store.create_run(Run(run_id="r1", goal="do X"))
    store.append_event("r1", EventType.RUN_STARTED, {"goal": "do X"}, source=Origin.EXTERNAL_AGENT)
    store.append_event(
        "r1", EventType.TASK_UPDATED, {"completed": 1, "failed": 0}, source=Origin.EXTERNAL_AGENT
    ...
    store.compact_run("r1")
    # The silence event is archived out of the live tail by the compaction.
    assert not any(
...
def test_consumed_authority_survives_compaction(store: SQLiteStorage) -> None:
    """An authority consumed before the anchor must still block resume once
    ...
    store.compact_run("r1")
    # The authority event is archived out of the live tail by the compaction.
