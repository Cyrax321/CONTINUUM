...
try:
    ...
    # First launch for this DB: seed the run. Anything else is a real failure
    # and must not be swallowed mid-batch.
    resumed = False
    store.create_run(Run(run_id="run_4821", goal="Analyze 1,000 documents"))
    store.append_event(
        "run_4821", EventType.RUN_STARTED, {"goal": "Analyze 1,000 documents", "total": 1_000}
    )
    store.append_event(
        "run_4821", EventType.DEPENDENCY_DECLARED, {"resource": "dataset", "version": dataset}
    )
    store.append_event(
        "run_4821",
        EventType.EVIDENCE_ADDED,
        {"evidence_id": "paper_128", "summary": "peer-reviewed study", "source": "dataset"},
    )
    store.append_event(
        "run_4821",
