...
- **Tamper of history**  
  The event log is hash chained and checkpoints are sealed. The recovery ledger is hash chained from `GENESIS` via `stable_hash`. `verify` and `verify_contract` detect any edit of a prior entry by hash mismatch. See `src/continuum/recovery/contract.py:66` and `src/continuum/recovery/ledger.py:377`.

- **Replay of stale state**  
  Replaying an old checkpoint that is behind a sealed contract is detected as drift by `RecoveryLedger.reconcile`, which compares live `state.version` to the ledger high watermark `max(contract.checkpoint_version)`. See `src/continuum/recovery/ledger.py:523`. Tested in `tests/test_ledger_replay.py:1`.
...
- **Body-size exhaustion on HTTP transports**  
  Denial of service through unbounded reads is fended off by strict body caps across all HTTP transports. The gateway, dashboard, and serve transports fail closed: requests declaring a `Content-Length` over the transport body cap (10 MB for `gateway.py`, 1 MB for `dashboard/app.py` and `serve/server.py`) are refused with `413 Payload Too Large` before the body is read into memory. To prevent broken pipes, transports drain-and-discard up to a shared 256 MB bound; when draining exceeds this limit, the server closes the connection immediately. See `src/continuum/gateway.py:73`, `src/continuum/dashboard/app.py:18`, and `src/continuum/serve/server.py:76`. Tested in `tests/test_gateway.py:275`, `tests/test_dashboard.py:157`, and `tests/test_serve_http.py:562`. The proxy's reply half is capped too, since a proxy that only bounds what a client can make it hold is still hostage to what an upstream sends it: a reply whose declared `Content-Length` passes `MAX_RESPONSE_BYTES` (10 MB, the request cap's sibling) is refused with `502` before it is buffered, and a reply that declares no length at all is bounded by the running total rather than read whole, which is the only way a chunked reply can be bounded at all. The claim settles uncertain rather than completed, since an upstream that cannot answer inside the cap may still have applied the side effect. See `src/continuum/gateway.py:83` and `src/continuum/gateway.py:90`. Tested in `tests/test_gateway.py:505` and `tests/test_gateway.py:525`.
