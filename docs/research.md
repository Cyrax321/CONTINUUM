...
- **Recovery-correctness suite (Phase 6)** - `src/continuum/benchmark/phase6/` encodes crash points from the durable-execution survey as executable assertions. It is the 14-scenario suite referenced in `references/bench.md` and `STATUS.md`. Run via `pytest` (see `tests/test_benchmark.py` family).
