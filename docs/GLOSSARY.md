...
- **Contract**  
  The machine readable verdict `RecoveryContract` (`src/continuum/models.py:1114`) sealed by hash. It carries `recovery_status`, `checkpoint_version`, `verified` and `invalidated` lists, `required_actions`, a single `next_allowed_action`, plus the Phase 1 fields `evidence` and `reason`. Built by `build_contract` in `src/continuum/recovery/contract.py:88` and surfaced by `RecoveryEngine.assess`. The contract is the single permitted next step. Rendering is in `src/continuum/recovery/contract.py`.

- **Provenance**  
  The three orthogonal axes that trace a fact back to its origin: `Origin` (who asserted it, `src/continuum/models.py:192`), `TrustLevel` (how verified it is, `src/continuum/security/provenance.py:19`), and `StateStatus` (what validity state it is in, `src/continuum/models.py:97`). `src/continuum/provenance_map.py:60` projects these onto `CanonicalProvenance` without deleting the source enums. `ProvenanceView` carries all three source values alongside the canonical labels.

- **Checkpoint**  
  A sealed `StateCheckpoint` (`src/continuum/models.py:1188`) bundling the projected `SemanticState` at a version, the event cursor it covers, and the `EnvironmentSnapshot` it was verified against. Created in `src/continuum/checkpoint/manager.py:174` via `checkpoint`, restored via `restore` which replays events recorded after the checkpoint. The checkpoint policy that decides when to write lives in `src/continuum/checkpoint/policy.py:52`.

- **EnvironmentSnapshot**  
  A capture of the current external world for a run (`src/continuum/models.py:1072`). Each `EnvResource` (`src/continuum/models.py:1060`) records a named resource and its version or checksum. Comparison of a checkpoint environment and a live environment drives `EnvironmentDiff` and the validator's staleness propagation.

- **Validation**  
  `StateValidator` in `src/continuum/state/validator.py:222` decides per component whether the checkpointed state is still true against the live environment. Staleness propagates `dependency -> evidence -> finding -> decision`. This propagation is mirrored by the `DependencyGraph` in `src/continuum/recovery/impact.py:48` for localized repair.
