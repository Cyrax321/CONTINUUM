...
| Environment Model | PARTIAL | `environment/snapshot.py:47`, `environment/config.py` | `EnvironmentProvider` ABC exists; File/Git/Value/Callable/Static providers. Missing: Database/Browser/Cloud/GitHub providers. Providers were passed manually to `capture()` with no discovery; as of #762 a run records configured providers and their bounded scopes in the event log and `RecoveryEngine` resolves built-in or registered providers at resume, failing closed per status. Plugin auto-discovery is deliberately rejected. |
